...
import {
  ...
  createTemplateTaskRequestSchema,
  createMonthlyPlanRequestSchema,
...
const handleApiError = (error: unknown) => {
  if (error instanceof z.ZodError) {
    ...
    console.error("API response validation failed", error, requestDetails);
    return;
  }
...
const api = {
  ...
  households: {
    ...
    create: async (name: string): Promise<HouseholdResponse> => {
      ...
      const response = await axios.post<{ household: HouseholdResponse }>(
        "/households",
        validatedRequest,
      );
    ...
    update: async (id: number, name: string): Promise<HouseholdResponse> => {
      const validatedRequest = createHouseholdRequestSchema.parse({ name });
      const response = await axios.put<{ household: HouseholdResponse }>(
        `/households/${id}`,
        validatedRequest,
      );
      const validatedData = z.object({ household: householdSchema }).parse(response.data);
    ...
    addMember: async (
      ...
      const response = await axios.post<{ member: HouseholdMember }>(
  ...
  categories: {
    getByHousehold: async (householdId: number): Promise<CategoryResponse[]> => {
      try {
        const response = await axios.get<{ categories: CategoryResponse[] }>(
          `/categories/household/${householdId}`,
        );
    ...
    create: async (
      ...
      const response = await axios.post<{ category: CategoryResponse }>(
        "/categories",
        validatedRequest,
      );
    ...
    update: async (
      ...
      const response = await axios.put<{ category: CategoryResponse }>(
        `/categories/${id}`,
        validatedRequest,
      );
      const validatedData = z.object({ category: categorySchema }).parse(response.data);
  ...
  templates: {
    getByHousehold: async (householdId: number): Promise<TemplateResponse[]> => {
      const response = await axios.get<{ templates: TemplateResponse[] }>(
        `/templates/household/${householdId}`,
      );
    ...
    create: async (
      ...
      const response = await axios.post<{ template: TemplateResponse }>(
        "/templates",
        validatedRequest,
      );
    ...
    addTask: async (
      ...
      const response = await axios.post<{ task: TemplateTaskResponse }>(
    ...
    updateTask: async (
      ...
      const validatedRequest = createTemplateTaskRequestSchema.parse(task);
      const response = await axios.put<{ task: TemplateTaskResponse }>(
  ...
  monthlyPlans: {
    getByHousehold: async (householdId: number): Promise<MonthlyPlanResponse[]> => {
      try {
        const response = await axios.get<{ plans: MonthlyPlanResponse[] }>(
          `/monthly-plans/household/${householdId}`,
        );
    ...
    create: async (data: CreateMonthlyPlanRequestType): Promise<MonthlyPlanResponse> => {
      try {
        ...
        const response = await axios.post<{ plan: MonthlyPlanResponse }>(
          "/monthly-plans",
          validatedRequest,
        );
    ...
    addTask: async (task: CreateTaskRequestType): Promise<TaskResponse> => {
      ...
      const response = await axios.post<{ task: TaskResponse }>(
        "/monthly-plans/tasks",
        validatedRequest,
      );
    ...
    completeTask: async (taskId: number, isCompleted: boolean): Promise<TaskResponse> => {
      const validatedRequest = completeTaskRequestSchema.parse({ isCompleted });
      const response = await axios.patch<{ task: TaskResponse }>(
        `/monthly-plans/tasks/${taskId}/complete`,
        validatedRequest,
      );
      const validatedData = z.object({ task: taskSchema }).parse(response.data);
      return validatedData.task;
    ...
    closePlan: async (id: number): Promise<MonthlyPlanResponse> => {
      const response = await axios.patch<{ plan: MonthlyPlanResponse }>(
        `/monthly-plans/${id}/close`,
      );
      const validatedData = z.object({ plan: monthlyPlanSchema }).parse(response.data);
      return validatedData.plan;
    },
